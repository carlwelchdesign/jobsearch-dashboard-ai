...
`JobPosting.applicationUrl` is reserved for direct employer or ATS application targets. Search-query source, detail, board, listing, auth, paywall, and intermediary URLs are preserved in `rawData` for audit and repair, but they are not launchable Apply Sprint URLs unless the adapter or repair service resolves them to a direct external apply URL.
...
Search-query results are conservatively screened for list/search pages before they enter scoring. Known expandable pages, such as Built In job lists, are parsed into individual job postings. Strong listing signals, such as filtered job-board URLs with `page`, `sort`, `jobTitle`, or similar query parameters, are never saved as ordinary jobs when they cannot be expanded; they are recorded in search-run progress for review. Board and intermediary detail URLs from Built In, Working Nomads, Wellfound, Remote Rocketship, Remotive, RemoteOK, Dice, Indeed, ZipRecruiter, and similar providers can produce reviewable job records, but they do not become `applicationUrl` values until a direct employer/ATS apply link is found.
