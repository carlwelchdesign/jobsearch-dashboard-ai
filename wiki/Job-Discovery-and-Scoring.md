...
The seeded `Broad LinkedIn Parity Review` profile is a wide discovery lane for senior software, full-stack, frontend, React, TypeScript, product engineer, design-system, AI product UI, remote, US, and global roles. It uses a lower review threshold and higher per-run cap than the focused profiles. Lower-confidence broad matches are saved with a `Review-only broad discovery` recommended-action marker, remain visible in Jobs and Apply Sprint diagnostics, and are excluded from automatic recruiting-agency preparation until manually approved.
