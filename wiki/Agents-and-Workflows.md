...
Slack Agent Ops is a notification and approval surface, not a third agent runtime. The Socket Mode worker posts redacted summaries for completed Jolene Chief of Staff, Jolene Operating Loop, and Recruiting Search Team runs, and its buttons call the same internal approval/apply/rollback services used by the app. Slack deliveries are logged as `NotificationLog` type `slack`, and search-profile actions add `AgentRunEvent` records when tied to an agent run.
