...
Use `config/slack-app-manifest.example.yml` as the starting Slack app manifest. Install or reinstall the app after adding Home tab/event subscription support, channel/group message events, and the `channels:history`/`groups:history` scopes, invite it to the configured channels, then restart `npm run slack:dev`. If `/jso` works but normal messages in the configured Jolene channel do not trigger threaded replies, the installed app is missing those history scopes or has not been reinstalled since they were added.
...
Search analytics charts appear on Command Center, Runs, Sources run controls, and Apply Sprint. Use the outcome board and supporting charts to debug high raw volume with low application yield: the panel highlights save rate, qualification rate, top blocker, best source, best profile, outcome mix, source yield, profile yield, score quality bands, and recent qualified/saved/agency-eligible trends. Common blockers are below-threshold scoring, duplicate/existing matches, profile `maxResultsPerRun` caps, suppressed listing pages, missing application URLs, provider-missing warnings, and review-only broad discovery matches. The four persisted counters remain fixed on `JobSearchRun`; all extra diagnostics are progress JSON so no migration is required.
