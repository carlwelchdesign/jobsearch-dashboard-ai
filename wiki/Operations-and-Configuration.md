...
- `AgentImprovementProposal` acceptance is controlled. Low-risk mapped proposals create active `SkillAdjustment` guidance; high-risk, unmapped, prompt, scoring-policy, search-source, and workflow proposals remain review-only and do not rewrite prompts, code, or workflow policy automatically.
