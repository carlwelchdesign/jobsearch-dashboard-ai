...
Resume profile role history now supports recruiter-format experience context. Pasted role descriptions can populate application/product title, what the application did, who used it, scale or impact, confirmed tech, and review-only version suggestions derived from role dates or explicit source evidence. Generated resumes render `Company - Title | Dates`, optional quoted application title, context, outcome bullets, and `Tech Used:` lines, but exact versions appear only after they are confirmed or approved in `/resumes/profile`; unapproved inferred version language is flagged by material QA.
...
Apply Sprint starts with the next ready application instead of the diagnostic funnel. The primary path shows the selected application, application link, packet readiness, assistant launch, cover-letter copy, manual applied confirmation, and reject/remove action. Candidates, agency results, hidden/suppressed rows, queue progress, reset controls, and run logs stay available behind details panels so the user can diagnose issues without losing the daily apply flow.
