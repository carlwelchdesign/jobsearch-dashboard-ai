...
The LinkedIn Content studio at `/linkedin-content` now works as an agent content team. It builds a public-safe memory and analytics pack from recent `AgentRun` records, search-run analytics, Apply Sprint/application aggregate counts, Market Intelligence context, active learning rules, prior drafts, post performance analytics, and screenshot metadata. The content team includes documentarian, analytics narrator, product strategist, editor, visual producer, privacy reviewer, and publisher roles. Agents can use aggregate numbers such as jobs fetched, scored, qualified, saved, drop-off reasons, application status counts, source coverage, workflow learning, impressions, reach, engagement, and content-pillar performance, but public posts cannot include company names, job URLs, salaries, recruiters, emails, application-specific outcomes, viewer identities, or commenter identities. Drafts expose agent reviews, source provenance, grounded claims, editable copy, subtle agent-team disclosure, and real redacted Playwright screenshots from allowlisted app routes. Clicking **Approve and publish** is the final user action: if privacy/provenance checks pass and LinkedIn publishing is connected, the app immediately creates the LinkedIn UGC post and records the returned post id or a retryable publish error.
