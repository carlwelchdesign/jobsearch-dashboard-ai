...
Settings also supports optional LinkedIn OIDC connection through `LINKEDIN_CLIENT_ID`, `LINKEDIN_CLIENT_SECRET`, and `LINKEDIN_OIDC_REDIRECT_URI`. It requests only `openid profile email`, imports durable profile basics such as name, email, photo, locale, subject, and email verification status, and does not store LinkedIn access tokens. This connection does not unlock LinkedIn job search, saved jobs, Apply with LinkedIn, Apply Connect, Job Posting APIs, scraping, or automated LinkedIn browsing.

The app also includes a draft-only LinkedIn Content agent at `/linkedin-content`. It turns app progress, source coverage, search-learning, architecture choices, and workflow lessons into reviewable LinkedIn post drafts with safe aggregate screenshot-style attachments. The agent does not request `w_member_social`, does not post through the Share on LinkedIn API, and blocks screenshot downloads unless the generated asset passes privacy review for names, emails, companies, salaries, job URLs, and application-specific data.
