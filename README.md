...
Settings supports two LinkedIn connections using `LINKEDIN_CLIENT_ID` and `LINKEDIN_CLIENT_SECRET`. The identity callback at `LINKEDIN_OIDC_REDIRECT_URI` requests `openid profile email`, imports durable profile basics such as name, email, photo, locale, subject, and email verification status, and does not store identity access tokens. The publishing callback at `LINKEDIN_SHARE_REDIRECT_URI` requests `openid profile email w_member_social`, stores a separate `LinkedInShareConnection`, and can publish approved `/linkedin-content` drafts through Share on LinkedIn. Neither connection unlocks LinkedIn job search, saved jobs, Apply with LinkedIn, Apply Connect, Job Posting APIs, scraping, or automated LinkedIn browsing.

The LinkedIn Content studio at `/linkedin-content` now works as an agent content team. It builds a public-safe memory and analytics pack from recent `AgentRun` records, search-run analytics, Apply Sprint/application aggregate counts, Market Intelligence context, active learning rules, prior drafts, and screenshot metadata. The content team includes documentarian, analytics narrator, product strategist, editor, visual producer, privacy reviewer, and publisher roles. Agents can use aggregate numbers such as jobs fetched, scored, qualified, saved, drop-off reasons, application status counts, source coverage, and workflow learning, but public posts cannot include company names, job URLs, salaries, recruiters, emails, or application-specific outcomes. Drafts expose agent reviews, source provenance, grounded claims, editable copy, subtle agent-team disclosure, and real redacted Playwright screenshots from allowlisted app routes. Clicking **Approve and publish** is the final user action: if privacy/provenance checks pass and LinkedIn publishing is connected, the app immediately creates the LinkedIn UGC post and records the returned post id or a retryable publish error.
