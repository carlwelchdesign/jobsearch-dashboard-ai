...
Search run analytics are chart-first on Command Center, Runs, Sources run controls, and Apply Sprint. The search analytics panel now starts with an outcome board instead of a linear conversion funnel: fetched, qualified, saved, agency-eligible, save rate, top blocker, best source, and best profile are visible at a glance. Supporting charts show the run outcome mix, prioritized blockers, ranked profile yield, source yield as volume-versus-quality, quality score bands, and recent qualified/saved/agency-eligible trends. The four persisted counters remain fixed on `JobSearchRun`; richer analytics still come from progress JSON, so no migration is required.
