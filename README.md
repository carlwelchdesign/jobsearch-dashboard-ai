...
The resume profile is a Job Evidence Library organized by role timeline instead of separate context and bullet lists. Each job owns its verified/proposed bullets, application/product context, confirmed tech, version suggestions, source records, and duplicate cleanup actions. Pasted role descriptions can backfill a specific job, resume-upload bullets are linked to matching work history when approved, and duplicate work rows are grouped for explicit review before merge. Generated resumes render each Professional Experience entry as `Company - Title | Dates`, `Skills: ...`, then truthful outcome bullets. Exact technology versions appear only after they are confirmed or approved in `/resumes/profile`; unapproved inferred version language is flagged by material QA. `/settings/application` includes a live resume format preference for generated resume previews and PDF exports, defaulting to an ATS-safe modern two-column visual layout while leaving stored markdown and plain text unchanged. The `ATS_RESUME_REVIEWER` runs after job-specific resume generation to check ATS readability, recruiter red flags, keyword coverage, incomplete profile links, and resume formatting; clear high-confidence fixes can auto-rewrite the generated resume while preserving the original in metadata. Uploading a resume now routes directly to `/resumes/review`, where the user can re-parse bad extracted data before approval. Approving a new resume acts as re-onboarding: the latest approved upload becomes the active resume source, candidate intelligence records review evidence, and the Search Profile Manager returns reviewable profile suggestions without creating or deleting profiles automatically.
...
Apply Workspace is the canonical surface after a job has an application tracker. Opening a tracked job hands off to `/applications/[id]`, where the first viewport shows status, match evidence, packet readiness, direct application link, visible blockers, and one recommended next action. The workspace prepares, reviews, and assists applications; the user performs final submission. It can launch the local assistant to fill fields, upload materials, and stop at the final review screen, but it does not click Submit, contact employers, send unapproved email, write calendar events, or publish LinkedIn content. Supporting material stays available in workspace sections for Materials, Answers, Fit, Research, History, and Job details so the user can diagnose issues without losing the daily apply flow. Jobs with board, listing, auth, paywall, or intermediary URLs show as `unsupported_application_url` in diagnostics until a direct employer or ATS target is added or repaired. Applications with fallback or weak cover letters show as `material_quality_needs_review` and remain approved/review-only until regenerated or manually repaired.
...
Some job boards require Google OAuth, human verification, or paid apply flows. Those sources are disabled by default, suppressed when unresolved, or treated as manual-only: the assistant opens the job in your normal browser and reveals the prepared materials folder instead of trying to automate the login. Remotive leads are query-covered, alternate-resolved when possible, and otherwise suppressed for review instead of being promoted to direct application URLs.
