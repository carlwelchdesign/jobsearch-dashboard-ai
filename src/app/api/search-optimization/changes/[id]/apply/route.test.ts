...
const applyMock = vi.mocked(applySearchProfileChange);
const findFirstMock = vi.mocked(prisma.searchProfileChange.findFirst);

describe("POST /api/search-optimization/changes/[id]/apply", () => {
  beforeEach(() => {
    ...
    applyMock.mockResolvedValue({ id: "change_1", status: "APPLIED" } as never);
  });

  it("applies a change owned by the protected user", async () => {
    const response = await POST(new Request("http://localhost/api/search-optimization/changes/change_1/apply", { method: "POST" }), { params: { id: "change_1" } });

    expect(response.status).toBe(200);
    expect(findFirstMock).toHaveBeenCalledWith({ where: { id: "change_1", userId: "user_1" }, select: { id: true } });
    expect(applyMock).toHaveBeenCalledWith("change_1");
  });
