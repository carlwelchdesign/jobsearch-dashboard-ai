...
export const skillRegistry = {
  ...
  application_evidence_curator: {
    id: "application_evidence_curator",
    label: "Application Evidence Curator",
    agentType: "APPLICATION_EVIDENCE_CURATOR",
    riskLevel: "LOW",
    inputSchema: z.object({ jobPostingId: z.string(), jobSearchProfileId: z.string().nullable().optional(), ...optionalUser }),
    outputSchema: anyOutput,
    defaultPolicy: lowRiskPolicy,
    execute: async (input: any) => (await (await import("@/lib/agents/application-evidence-curator")).runApplicationEvidenceCuratorAgent(input)).output,
  },
  hiring_manager_reviewer: {
    id: "hiring_manager_reviewer",
    label: "Hiring Manager Reviewer",
    agentType: "HIRING_MANAGER_REVIEWER",
    riskLevel: "LOW",
    inputSchema: z.object({
      jobPostingId: z.string(),
      userId: z.string().optional(),
      coverLetterBody: z.string(),
      generatedBy: z.string().nullable().optional(),
      evidencePlan: z.unknown().nullable().optional(),
      applicationQa: z.record(z.unknown()).nullable().optional(),
    }),
    outputSchema: anyOutput,
    defaultPolicy: lowRiskPolicy,
    execute: async (input: any) => (await (await import("@/lib/agents/hiring-manager-reviewer")).runHiringManagerReviewerAgent(input)).output,
  },
  job_fit_scorer: {
    id: "job_fit_scorer",
  ...
  recruiting_search_director: {
    id: "recruiting_search_director",
    label: "Recruiting Search Director",
    agentType: "RECRUITING_SEARCH_DIRECTOR",
    riskLevel: "LOW",
    inputSchema: z.object({ mode: z.enum(["diagnose_only", "active"]).optional(), ...optionalUser }),
    outputSchema: anyOutput,
    defaultPolicy: localMutationPolicy,
    execute: async (input: any) => (await (await import("@/lib/agents/recruiting-search-optimization")).runRecruitingSearchOptimization(input)).output,
  },
  search_yield_analyst: {
    id: "search_yield_analyst",
    label: "Search Yield Analyst",
    agentType: "SEARCH_YIELD_ANALYST",
    riskLevel: "LOW",
    inputSchema: z.object(optionalUser),
    outputSchema: anyOutput,
    defaultPolicy: lowRiskPolicy,
    execute: async (input: any) => (await (await import("@/lib/agents/recruiting-search-optimization")).buildSearchOptimizationContext(input)),
  },
  search_profile_editor: {
    id: "search_profile_editor",
    label: "Search Profile Editor",
    agentType: "SEARCH_PROFILE_EDITOR",
    riskLevel: "LOW",
    inputSchema: z.object(optionalUser),
    outputSchema: anyOutput,
    defaultPolicy: localMutationPolicy,
  ...
  source_quality_analyst: {
    id: "source_quality_analyst",
    label: "Source Quality Analyst",
    agentType: "SOURCE_QUALITY_ANALYST",
    riskLevel: "LOW",
    inputSchema: z.object(optionalUser),
    outputSchema: anyOutput,
    defaultPolicy: lowRiskPolicy,
  ...
  match_calibration_reviewer: {
    id: "match_calibration_reviewer",
    label: "Match Calibration Reviewer",
    agentType: "MATCH_CALIBRATION_REVIEWER",
    riskLevel: "LOW",
    inputSchema: z.object(optionalUser),
    outputSchema: anyOutput,
    defaultPolicy: lowRiskPolicy,
  ...
  outcome_recruiter: {
    id: "outcome_recruiter",
    label: "Outcome Recruiter",
    agentType: "OUTCOME_RECRUITER",
    riskLevel: "LOW",
    inputSchema: z.object(optionalUser),
    outputSchema: anyOutput,
    defaultPolicy: lowRiskPolicy,
