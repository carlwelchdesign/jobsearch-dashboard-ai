...
function likelyProfileFit(job: RawJobPosting, company: CompanySource, profile: JobSearchProfile) {
  ...
  const softwareTitle = /\b(frontend|front-end|full.?stack|software engineer|product engineer|ui engineer|web engineer|react|typescript|next\.?js|design systems?|frontend platform|developer experience|internal tools|admin console|enterprise ui|mission software|human-machine interface|visualization engineer|applied ai|ai product|developer tools?)\b/i.test(title);
  ...
  const negative = /\b(intern|junior|new grad|early career|ios only|android only|firmware|embedded|wordpress|php|quantitative researcher|ml research scientist|production coordinator|quality inspector|accounts payable|mechanical engineer|aerodynamics engineer|electrical engineer|electrical test engineer|emi\/emc|flight sciences engineer|flight test engineer|field operations engineer|endpoint systems engineer|business operations engineer|deputy chief engineer|developer success engineer|customer success|solutions engineer|facilities|recruiter|sales|account executive)\b/i.test(title);
